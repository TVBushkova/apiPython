...
class TestNewLocation():
    ...
    def test_create_new_location(self):
        ...
        key = '?key=qaclick123' #Параметр для всех запросов
        put_recourse = '/maps/api/place/update/json' #Ресурс для метода PUT

        put_url = base_url + put_recourse + key
        print(put_url)

        new_address = 'Bazhova 99-12'
        ...
        if result_put.status_code == 200:
            result_get = requests.get(get_url).json()
            final_put = result_get.get('address')
            print(f'Итоговый адрес: {final_put}')

        else:
            print('Не обновили локацию')
